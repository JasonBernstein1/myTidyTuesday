...
# This script plots the number of mice and rats over time by treatment.
...
df <- surveys |>
  left_join(species) |>
  mutate(
    mouse_or_rat = ifelse(str_detect(commonname, "mouse"), "mouse", "rat")
  ) |>
...
df |>
  ggplot() +
  geom_line(
    aes(x = year, y = n, col = str_to_sentence(treatment)),
    linewidth = 1.25
  ) +
  facet_wrap(
    ~mouse_or_rat,
...
ggsave(
  filename = here::here("images", "2023_week18.png"),
  height = 6,
  width = 9
)
