...
egg_prod <- tuesdata$`egg-production` |>
  rename(date = observed_month) |>
  # this is the variable we will plot over time
  mutate(
    egg_to_hen_ratio = n_eggs / n_hens
  )

# tabulate number of points by production process and type
with(egg_prod, table(prod_process, prod_type))
...
egg_prod |>
  ...
  select(date, prod_process, egg_to_hen_ratio) |>
...
df <- egg_prod |>
  ...
  mutate(
    ...
    prod_type = str_to_sentence(prod_type),
    prod_process = str_to_sentence(prod_process) |>
      # remove ' (organic)' from 'Cage-free (organic)'
      str_remove(" \\(organic\\)"),
    # combine production type and process for plotting
    prod_method = glue::glue("{prod_type} ({prod_process})") |>
...
df |>
  ggplot() +
  geom_line(
    aes(x = date, y = egg_to_hen_ratio, col = prod_method),
    linewidth = 1.5
  ) +
...
ggsave(
  filename = here::here("images", "2023_week15.png"),
  height = 10,
  width = 15
)
